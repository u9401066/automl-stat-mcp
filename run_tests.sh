...
if ! command -v uv &> /dev/null; then
    ...
    echo "請先安裝 uv，然後執行: uv sync --all-extras"
    exit 1
fi
...
if [ "$#" -eq 0 ]; then
    ...
    case $choice in
        1)
            run_test "uv run pytest -m 'not slow' tests/" "快速測試"
            ;;
        2)
            run_test "uv run pytest -m unit tests/" "單元測試"
            ;;
        3)
            run_test "uv run pytest -m integration tests/" "整合測試"
            ;;
        4)
            run_test "uv run pytest -m e2e tests/e2e/" "E2E 測試"
            ;;
        5)
            run_test "uv run pytest -m edge_case tests/edge_cases/" "邊界測試"
            ;;
        6)
            run_test "uv run pytest -m performance tests/performance/" "性能測試"
            ;;
        7)
            run_test "uv run pytest -m security tests/security/" "安全測試"
            ;;
        8)
            run_test "uv run pytest" "全部測試"
            ;;
        9)
            run_test "uv run pytest --cov=. --cov-report=html --cov-report=term tests/" "覆蓋率報告"
            echo ""
            echo -e "${GREEN}📊 HTML 報告已生成: htmlcov/index.html${NC}"
            ;;
        0)
            read -p "輸入 pytest 參數: " custom_args
            run_test "uv run pytest $custom_args" "自訂測試"
            ;;
        *)
            echo -e "${RED}無效選擇${NC}"
            exit 1
