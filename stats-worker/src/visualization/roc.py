...
def plot_roc_curve(
    ...
    if show_diagonal:
        ax.plot([0, 1], [0, 1], linestyle="--", color=ROC_COLORS["diagonal"], linewidth=1, label="Random (AUC = 0.5)")
    ...
    ax.text(0.02, 0.02, f"n+ = {n_pos}, n- = {n_neg}", transform=ax.transAxes, fontsize=9, va="bottom")
...
def plot_roc_curves_comparison(
    ...
    # Diagonal reference
    if show_diagonal:
        ax.plot([0, 1], [0, 1], linestyle="--", color=ROC_COLORS["diagonal"], linewidth=1, label="Random")
...
def plot_pr_curve(
    ...
    if show_baseline and baseline_precision is not None:
        ax.axhline(
            y=baseline_precision, linestyle="--", color="gray", linewidth=1, label=f"Random ({baseline_precision:.2f})"
        )
...
def plot_calibration_curve(
    ...
    if show_reference:
        ax_main.plot([0, 1], [0, 1], linestyle="--", color="gray", linewidth=1, label="Perfect calibration")

    # Plot calibration curve
    ax_main.plot(
        predicted_probs, observed_probs, marker="o", color=curve_color, linewidth=2, markersize=8, label="Model"
    )
...
def plot_threshold_analysis(
    ...
    for metric in metrics:
        if metric in ["sensitivity", "specificity", "ppv", "npv"]:
            ...
            if v_valid:
                ax.plot(
                    t_valid, v_valid, color=colors.get(metric, "gray"), linewidth=2, label=labels.get(metric, metric)
                )

    # Mark optimal threshold
    optimal_thresh = roc_result.get("optimal_threshold")
    if optimal_thresh is not None:
        ax.axvline(x=optimal_thresh, linestyle="--", color="gray", linewidth=1, label=f"Optimal ({optimal_thresh:.2f})")
