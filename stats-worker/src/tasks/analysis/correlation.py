...
class EnhancedCorrelationResult:
    ...
    pearson_matrix: Dict[str, Dict[str, Optional[float]]] = field(default_factory=dict)
    spearman_matrix: Dict[str, Dict[str, Optional[float]]] = field(default_factory=dict)

    # P-value matrices
    pearson_pvalue_matrix: Dict[str, Dict[str, Optional[float]]] = field(default_factory=dict)
    spearman_pvalue_matrix: Dict[str, Dict[str, Optional[float]]] = field(default_factory=dict)
...
def _matrix_to_dict(matrix: pd.DataFrame) -> Dict[str, Dict[str, Optional[float]]]:
...
def _compute_pvalue_matrix(df: pd.DataFrame, method: str) -> Dict[str, Dict[str, Optional[float]]]:
    """Compute p-value matrix for correlations."""
    cols = df.columns.tolist()
    pvalues: Dict[str, Dict[str, Optional[float]]] = {}

    for _i, col1 in enumerate(cols):
        pvalues[str(col1)] = {}
        for col2 in cols:
            if col1 == col2:
                ...
                try:
                    ...
                    pvalues[str(col1)][str(col2)] = safe_round(p, 4)
                except Exception:
                    pvalues[str(col1)][str(col2)] = None
