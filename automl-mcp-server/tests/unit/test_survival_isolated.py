...
class TestMedianSurvival:
    ...
    def test_median_from_km(self):
        ...
        median = None
        for t, s in zip(unique_times, survival, strict=False):
            if s <= 0.5:
                median = t
                break
...
class TestCensoringPatterns:
    ...
    def test_type1_censoring(self):
        ...
        np.minimum(true_times, study_end)
    ...
    def test_random_censoring(self):
        ...
        # Observed
        np.minimum(event_times, censor_times)
    ...
    def test_informative_vs_noninformative(self):
        ...
        np.minimum(event_times, censor_times)
        (event_times <= censor_times).astype(int)
