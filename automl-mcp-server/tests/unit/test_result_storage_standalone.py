...
# Pre-import real modules before mocking, so we can restore them later
...
mcp_server_fastmcp_mock.Context = object
...
# Import the module we need while mocks are active
from infrastructure.mcp.handlers.result_storage import (  # noqa: E402
    ResultMetadata,
    ResultStorage,
    get_result_storage,
    safe_json_dumps,
...
# Restore original modules so other tests aren't affected
for mod_name, original in _original_modules.items():
    sys.modules[mod_name] = original

# Fix the httpx binding in the already-imported result_storage module,
# since it was first imported while the mock httpx was active.
_rs_mod = sys.modules.get("infrastructure.mcp.handlers.result_storage")
if _rs_mod:
    _rs_mod.httpx = _real_httpx
