...
def register_statistics_tools(mcp: FastMCP, automl_client) -> None:
    ...
    @mcp.tool()
    async def list_user_visualizations(
        user_id: str,
        result_type: Optional[str] = None,
        limit: int = 50,
    ) -> dict:
        """
        🖼️ List visualizations (figures/plots) stored in MinIO for a user.

        Retrieves URLs to visualization files (PNG, SVG) generated by analysis tools.
        Visualizations are organized by job and analysis type.

        Args:
            user_id: User ID to list visualizations for
            result_type: Filter by type (roc, km_curve, correlation, etc.)
            limit: Maximum number of results (default 50)

        Returns:
            visualizations: List of visualization metadata with URLs
            count: Number of visualizations found

        Example:
            list_user_visualizations(user_id="eric", result_type="roc")
        """
        try:
            import httpx
    ...
    async def create_project_workspace(
        ...
        Creates organized directories for research projects:
        ...
        Args:
            project_name: Name of the project (alphanumeric + underscore)
            user_id: User ID for ownership
            template: Template type (default, medical_study, ml_project)

        Returns:
            project_path: Root path of the created project
            directories: List of created directories
            next_steps: Suggested next actions

        Example:
            create_project_workspace(
                project_name="breast_cancer_study",
                user_id="eric",
...
2. Clean data and save to `data/processed/`
3. Run analyses, results go to `analysis/`
...
"""
            ...
            return {
                ...
                "template": template,
                "next_steps": [
                    f"Upload data: upload_dataset(source_path='/data/projects/{project_name}/data/raw/your_data.csv')",
                    f"Preview: quick_preview(csv_path='/data/projects/{project_name}/data/raw/your_data.csv')",
                    f"Analyze: smart_analyze(csv_path='/data/projects/{project_name}/data/raw/your_data.csv')",
                ],
            }
        except Exception as e:
            return {"status": "error", "error": str(e)}
    ...
    async def list_project_workspaces(
        ...
        Shows all project directories in /data/projects/ with their metadata.

        Args:
            user_id: Optional filter by user (currently not enforced)

        Returns:
            projects: List of project directories with info
