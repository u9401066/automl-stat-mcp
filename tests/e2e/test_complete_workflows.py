...
注意：這些測試需要 MCP server 提供 REST 端點 (/mcp/tools/...)。
      目前 MCP server 使用 SSE 協議，這些端點不存在。
      設定 MCP_REST_AVAILABLE=1 環境變數以啟用。
"""
...
pytestmark = pytest.mark.skipif(
    not os.getenv("MCP_REST_AVAILABLE"),
    reason="MCP REST endpoints not available (server uses SSE protocol)",
)
    ...
    async def test_rct_full_pipeline(self, mcp_base_url, stats_base_url):
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            quality_response = await client.post(
                f"{mcp_base_url}/mcp/tools/quality_check", json={"csv_path": "/data/sample_data/medical_study_200.csv"}
            )
            ...
            tableone_response = await client.post(
                ...
                json={"csv_path": "/data/sample_data/medical_study_200.csv", "group_column": "treatment_group"},
            )
    ...
    async def test_survival_analysis_workflow(self, mcp_base_url):
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            km_response = await client.post(
                ...
                json={"csv_path": "/data/sample_data/rossi_recidivism.csv", "time_col": "week", "event_col": "arrest"},
            )
    ...
    async def test_ml_training_pipeline(self, mcp_base_url):
        ...
        async with httpx.AsyncClient(timeout=300.0) as client:
            ...
            leaderboard_response = await client.get(
                ...
                params={"model_id": model_id, "user_id": "e2e_test_user"},
            )
...
class TestMultiUserConcurrency:
    ...
    async def test_concurrent_analysis(self, mcp_base_url):
        ...
        async def analyze_for_user(user_id: str):
            async with httpx.AsyncClient(timeout=60.0) as client:
                response = await client.post(
                    f"{mcp_base_url}/mcp/tools/quick_stats", json={"csv_path": "/data/sample_data/iris.csv"}
                )
                return response.status_code, response.json()
