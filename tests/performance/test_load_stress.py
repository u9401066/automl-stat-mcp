...
注意：需要 MCP REST 端點。設定 MCP_REST_AVAILABLE=1 啟用。
"""
...
class TestLoadBenchmarks:
    ...
    async def test_large_dataset_performance(self, mcp_base_url):
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            response = await client.post(
                f"{mcp_base_url}/mcp/tools/smart_analyze", json={"csv_path": "/data/sample_data/medical_study_200.csv"}
            )
...
class TestConcurrentRequests:
    ...
    async def test_10_concurrent_previews(self, mcp_base_url):
        ...
        # 檢查結果
        for _session_id, status_code, _elapsed in results:
    ...
    async def test_large_result_handling(self, mcp_base_url):
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            for dataset in ["iris.csv", "titanic.csv", "breast_cancer.csv"]:
                response = await client.post(
                    f"{mcp_base_url}/mcp/tools/smart_analyze", json={"csv_path": f"/data/sample_data/{dataset}"}
                )
