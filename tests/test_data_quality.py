...
P005,MRN005,,999,1200,A,1"""
    ...
    def test_quality_warning_format(self, problematic_csv):
        ...
        expected_format = {
            "quality_warnings": [
                {"column": "all_nan", "issue": "ALL_NAN", "recommendation": "移除此欄或填補缺失值"},
                {"column": "constant", "issue": "CONSTANT", "recommendation": "移除此欄，無分析價值"},
                {"column": "patient_id", "issue": "HIGH_CARDINALITY_ID", "recommendation": "排除於分析外"},
                {"column": "income", "issue": "SKEWED", "recommendation": "考慮 log transform 或使用非參數方法"},
            ]
        }
