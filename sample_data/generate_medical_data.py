...
data = {
    ...
    'education': np.random.choice(
        ['High School', 'Bachelor', 'Master', 'PhD'], n, p=[0.30, 0.40, 0.20, 0.10]
    ),
}
...
# 收縮壓 (與年齡、BMI相關)
df['systolic_bp'] = (
    100 + df['age'] * 0.5 + df['bmi'] * 0.8 +
    np.random.normal(0, 12, n)
).round(0).astype(int)

# 舒張壓
df['diastolic_bp'] = (
    60 + df['age'] * 0.2 + df['bmi'] * 0.4 +
    np.random.normal(0, 8, n)
).round(0).astype(int)

# 空腹血糖 (mg/dL) - 右偏分佈
df['fasting_glucose'] = np.clip(
    np.random.gamma(10, 10, n) + 70, 65, 350
).round(0).astype(int)

# 總膽固醇 (mg/dL)
df['total_cholesterol'] = (
    150 + df['age'] * 0.8 + df['bmi'] * 1.5 +
    np.random.normal(0, 30, n)
).round(0).astype(int)

# HDL 膽固醇 (女性較高)
df['hdl_cholesterol'] = (
    45 + (df['gender'] == 'Female').astype(int) * 10 +
    np.random.normal(0, 12, n) -
...
df['ldl_cholesterol'] = (
    ...
    df['fasting_glucose'] / 5
).round(0).astype(int)

# HbA1c (與血糖相關)
df['hba1c'] = (
    4.5 + (df['fasting_glucose'] - 90) * 0.02 +
    np.random.normal(0, 0.5, n)
).round(1)
df['hba1c'] = np.clip(df['hba1c'], 4.0, 14.0)

# 肌酸酐 (mg/dL)
df['creatinine'] = np.clip(
    np.random.gamma(15, 0.07, n) + 0.5, 0.5, 4.0
).round(2)
